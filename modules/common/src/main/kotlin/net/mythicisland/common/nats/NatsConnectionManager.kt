...
class NatsConnectionManager(
    ...
    private val monitorJob: Job = scope.launch { monitorConnection() }
            ...
            when (val status = conn.status) {
                ...
                Connection.Status.RECONNECTING, Connection.Status.DISCONNECTED -> {
                    ...
                    if (reconnectDuration > Duration.ZERO &&
                        ...
                        reconnectingSinceMillis = null
                        reconnectCompletely("connection stayed $status for at least $reconnectDuration")
                    }
                }
        ...
        try {
            ...
            while (true) {
                val newConnection = runCatching {
                    ...
                    logger.warn("Failed to establish new NATS connection, retrying in $backoff: ${error.message}")
                    null
                }
                ...
                if (newConnection != null) {
                    ...
                    if (previousConnection !== newConnection) {
                        runCatching {
                            ...
                            logger.debug("Failed to close previous NATS connection", it)
                        }
                    }
                    logger.info("Successfully established a new NATS connection")
